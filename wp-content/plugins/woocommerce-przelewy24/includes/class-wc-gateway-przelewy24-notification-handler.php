...
class WC_Gateway_Przelewy24_Notification_Handler extends WC_Gateway_Przelewy24_Response {
    ...
    public function validate_ipn() {
        ...
        $params = array('body' => $validate_ipn, 'sslverify' => false, 'timeout' => 60, 'httpversion' => '1.1', 'compress' => false, 'decompress' => false, 'user-agent' => 'WooCommerce/' .  WC() -> version);
    ...
    private function payment_status_refunded($order, $posted) {
        ...
        if ($order -> get_total() == ($posted['mc_gross'] * -1)) {
            ...
            $this -> send_ipn_email_notification(sprintf(__('Payment for order #%s refunded/reversed', 'woocommerce'), $order -> get_order_number()), sprintf(__('Order %s has been marked as refunded - PayPal reason code: %s', 'woocommerce'), $order -> get_order_number(), $posted['reason_code']));
        }
    }
    ...
    private function payment_status_reversed($order, $posted) {
        ...
        $this -> send_ipn_email_notification(sprintf(__('Payment for order #%s reversed', 'woocommerce'), $order -> get_order_number()), sprintf(__('Order %s has been marked on-hold due to a reversal - PayPal reason code: %s', 'woocommerce'), $order -> get_order_number(), wc_clean($posted['reason_code'])));
    }
    ...
    private function payment_status_canceled_reversal($order, $posted) {
        $this -> send_ipn_email_notification(sprintf(__('Reversal cancelled for order #%s', 'woocommerce'), $order -> get_order_number()), sprintf(__('Order #%s has had a reversal cancelled. Please check the status of payment and update the order status accordingly here: %s', 'woocommerce'), $order -> get_order_number(), esc_url(admin_url('post.php?post=' . $order -> id . '&action=edit'))));
    }
